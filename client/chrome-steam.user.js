...
function updateHeroItemsList(){
  ...
  xhr.open("GET", "https://raw.github.com/bravo142/steamsearch/master/server/json/"+hero+".json", true);
  xhr.onreadystatechange = function() {
    if (xhr.readyState == 4) {
        var default_value = getParameterByName('hero_item');
        var el = $j("#select-hero_item");
        el.empty()
        $j.each(jQuery.parseJSON(xhr.response), function(key, value) {
          if(value['key'] == default_value)
            el.append($j("<option></option>").attr("value", value['key']).attr("selected", "selected").text(value['name']));
          else
            el.append($j("<option></option>").attr("value", value['key']).text(value['name']));
        });
    }
  }
  xhr.send();
